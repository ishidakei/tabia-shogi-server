...
//! which is the one place `%KACHI` departs from P-4's relay shape — and that
...
/// O-1's own published example, `7g7f 3c3d 2g2f`: three plies of setup, which
/// leaves **White** to move at `START`.
...
async fn a_declaration_is_judged_against_the_position_a_collection_entrys_setup_built() {
    ...
    // Out of turn: P-4's answer to a move from the side not to move, which
